...
axios.get(`https://api-1.miraiofficials123.repl.co/upt?add=${UrlRepl}`)
		logger(global.fca.languages.uptime + UrlRepl, '[ FCA-ASUNA ]');
	}
	catch {
		logger(global.fca.languages.erroUptime, '[ FCA-ASUNA ]');
  }
}
...
function buildAPI(globalOptions, html, jar) {
    ...
    logger(`${global.fca.languages.loginSu}${userID}`, "[ FCA-ASUNA ]");
    ...
    if (oldFBMQTTMatch) {
        ...
        logger(`${global.fca.languages.region}${region}`, "[ FCA-ASUNA ]");
    } else {
    ...
    var ctx = {
        ...
        loggedIn: true,
        access_token: 'X7AZ L74C ZFMH SU7N J53K 6KJL ENGA 6XDU',
        clientMutationId: 0,
...
logger(global.fca.languages.login, "[ FCA-ASUNA ]");
        return utils
            ...
            .then(function(res) {
                ...
                if (headers.location.indexOf('https://www.facebook.com/checkpoint/') > -1) {
                    logger(global.fca.languages.errorFa, "[ FCA-ASUNA ]");
                                            ...
                                            utils
                                                ...
                                                .then(res => {
                                                    try {
                                                        ...
                                                        clearInterval(checkVerified);
                                                        logger(global.fca.languages.okweb, "[ FCA-ASUNA ]");
                                                        if (callback === prCallback) {
                                                            callback = function(err, api) {
                                                                if (err) return prReject(err);
                                                                return prResolve(api);
...
require('dotenv').config({ path: './../.env' });
                ...
                var ans = makeid(49)
                    process.env["FBKEY"] = ans;
                        fs.writeFile('./../.env', `FBKEY=${ans}`, function (err) {
                            if (err) {
                            logger(global.fca.languages.errorEnv, "[ FCA-ASUNA ]");
                    }
                else logger(global.fca.languages.env,"[ FCA-ASUNA ]")
        }); 
    }
    catch (e) {
        console.log(e);
        logger(global.fca.languages.errorPassRandom, "[ FCA-ASUNA ]");
    }
}
    ...
    if (process.env['FBKEY']) {
        try {
            ...
            if (appState.includes('[')) {
                logger(global.fca.languages.appstateDec, '[ FCA-ASUNA ]');
            } else {
                try {
                    appState = JSON.parse(appState);
                    var StateCrypt = require('./StateCrypt');
                    appState = StateCrypt.decryptState(appState, process.env['FBKEY']);
                    logger(global.fca.languages.appstate, '[ FCA-ASUNA ]');
                }
                catch (e) {
                    logger(global.fca.languages.appstateError, '[ FCA-ASUNA ]');
                }
            }
        }
        catch (e) {
    ...
    try {
        ...
        try {
            ...
            return logger(global.fca.languages.appstateError, '[ FCA-ASUNA ]')
        }
    }
      }
    try { 
    ...
    mainPromise
        .then(function() {
            logger(global.fca.languages.oklogin, "[ FCA-ASUNA ]");
    ...
    axios.get('https://raw.githubusercontent.com/tivutru/bot-asuna-temp/master/Update-1.10.0').then(async (res) => {
        const localbrand = JSON.parse(readFileSync('./node_modules/bot-asuna-temp/package.json')).version;
            if (localbrand != res.data.version) {
                    ...
                    logger(`${global.fca.languages.checkVersion}` + localbrand + ' !', "[ FCA-ASUNA ]");      
                         logger(global.fca.languages.chucAdmin, "[ FCA-ASUNA ]");
